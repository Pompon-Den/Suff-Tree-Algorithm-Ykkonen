...
Первоначальная идея алгоритма Укконена для построения суффиксного дерева  можно представить как простой,
...
Позже были изобретены другие, более эффективные алгоритмы построения суффиксного дерева, например обновленный алгоритм Питера Вайнера.


--------------------------------------

### Задача Online поиск шаблонов в тексте <p id = "Z"></p>
Дана некоторая строка(техт) __T__, которую можно прочитать и предобработать. Дан ряд подстрок исходной строки(техт) __T__.

 Требуется за длину подстроки  __***O***(|P|)__ определить ,
 находиться ли каждая из подстрок в строке. При этом следует избегать многократного просмотра строки.  [[17]](https://compscicenter.ru/courses/algorithms-2/nsk/2018-spring/classes/3764/)

---------------------------
### Основная идея алгоритма заключается в нескольких модификациях бора <p id = "M"></p>
- Использование Сжатого суффиксного дерева.
- Хранение подстрок не полностью, а лишь  индексов начала и конца подстроки относительно исходной строки.
...
<div>Рис.1 Бор на суффиксах</div>
</center> 



Размер такой структуры слишком большой, чтобы быстро отвечать на запрос поиска подстроки.
...
Для уменьшения по затратам памяти, хранить их будем в виде **[start...end]**,
 где **start** - индекс начало подстроки в тексте, **end** - индекс конца подстроки в тексте. [[17]](https://compscicenter.ru/courses/algorithms-2/nsk/2018-spring/classes/3764/)

<center> 

![Second](Image/Second.JPG)
<div>Рис.2 Сжатый суффиксный бор</div>
</center> 
...
- ` Определение` — это ориентированное дерево с корнем,
...
Но определение сжатого суффиксного дерева для S не гарантирует, что такое дерево действительно существует для любой строки S.
...
Можно заметить, что положения суффиксов бывают неявными, чтобы это исправить построим дерево для
 конкатенации исходной строки и терминального символа, 
...
<div>Рис.4 Полный вид  сжатого суффиксного дерева</div>
</center> 

------------------------------------

### Алгоритм построения <p id = "A"></p>
...
<div>Рис.5 Добавления символа</div>
</center> 

Итераций (шагов) - будет линейное количество, равное длине строки.
...
<div>Рис.6 Шаг внутри итерации</div>
</center> 
...
<div>Рис.7 Положение первого типа</div>
</center> 

При добавлении символа в лист, достаточно расширить дугу которая в этой лист идёт,
...
Заметим,что первый тип всегда преобразуется в самого себя т.е на каждой итерации  удлиняются существующие листья.
...
При втором типе,  добавляется новая вершина, а так как вершин у дерева линейное количество,
...
Т.к.  положения  перебираются в порядке уменьшения глубины то заметим, что номера типов положений не убывают:**« 1,..., 1, 2,...,2, 3,..., 3 »**.
Значит как только встретилось положение типа 3, осуществляется переход к следующей итерации.
...
<div>Рис.8 Положение типа три</div>
</center> 

Раз перебираются  положения от более глубоких к менее глубоким, то при переходе  попадаем в суффикс короче на один символ.
...
<div>Рис.9 Изменение положений</div>
</center> 
...
- Увеличиваться же потенциал будет на 1 при переходе на следующую итерацию, поскольку глубина в таком случае увеличивается на 1 символ.
 ...
 Следовательно алгоритм посетил линейное количетво положений.
...
- **`Определение`** - Пусть __xα__ обозначает произвольную строку, где __x__ — её первый символ,
                                а __α__ — оставшаяся подстрока (возможно пустая).
                                Если для внутренней вершины __v__ с хранящимся внутри  __xα__ существует другая вершина __s(v)__ с хранящимся внутри __α__,
                                то ссылка из __v__ в __s(v)__ называется суффиксной ссылкой.
...
Для вершин с положением  типа 1, суффиксная ссылка не изменяется.

Для положения типа 2, поскольку образуется новая вершина,  запоминается существующая суффиксная ссылка,
 чтобы в дальнейшем используя её создать у новой вершины суффиксную ссылку.
И когда на **i**-ой итерации создаётся новая(не первая) вершина для этой итерации, то она соединяется с созданной до этого вершиной суффиксной ссылкой.
...
Легко увидеть, что в процессе построения суффиксного дерева уже построенные суффиксные ссылки никак не изменяются.
 ...
 То есть суффиксные ссылки будут обновляться с запаздыванием. 

Внимательно посмотрев на все три правила продления суффиксов,
...
### Вершина(узел) Node суффиксного дерева включает:
...
- __`int activeLength`__ - количество символов ступаем в направлении activeEdge
- __`int remainingSuffixCount`__ - количество суффиксов осталось создать. по сути, сколько суффиксов на прошлом шаге  не создали.
...
- __`void BuildSuffixTree()`__ - построение дерева.
- __`~SuffixTree()`__ - деструктор.
- __`void DeleteSuffixTree(Node* node)`__ - удаление дерева.
- __`void ExtendSuffixTree(int pos)`__ - расширение дерева(добавления символа).
- __`int EdgeLength(Node* node)`__  - подсчет длины подстроки расположенной на дуге.  
- __`bool Find(string Pstr);`__  - поиск подстроки в строке.  
- __`void PrintSuffixs();`__  - вывод всех суффиксов дерева.  
- __`void GetSuffixIndexs(Node* node, vector<int>& vec);`__ - сбор индексов суффиксов в вектор.
-------------------------------------------------------------------------------


## Оценка производительности <p id = "time"></p>
...
В течение работы алгоритма создается не более **O(n)** вершин. Все суффиксы, которые заканчиваются в листах,
 благодаря  первому правилу на каждой итерации увеличиваем на текущий символ по умолчанию за **O(1)**.
 ...
 На каждой фазе начинаем добавление суффикса не с корня, а с индекса __j∗__, на котором в прошлой фазе было применено правило 3,
...
![11](Image/chart.png)
<div>Рис.11 График производительности построения дерева</div>
 </center>
...
<div>Рис.12  График производительности построения дерева на больших данных</div>
 </center>


### Вывод
 Тестироавание показало производительность **O(n)**

### Оценка времени работы поиска подстроки в строке в сравнении с встроенной функцией **find**.

График на малых строках. По графику видно, что поиск в дереве проигрывает встроенному **find**. Следовательно на малых данных он эффективней.
<center> 
...
В папке проекта на github находится папка Test, после генерации в ней хранятся все тесты.

 По скольку у файлов .txt со сгенирируемымии строками достаточно большой объем,
 они в репозитории не хранятся, а генерируется на пк пользователя.
В репозитории хранится только файл **manualTests.txt** с ручными тестами.

Для генерации тестов предусмотренны две функции в main.cpp, а именно 

```c++
/*Генерирует в файл с именем filename  countString случайных строк длиной в диапозоне от lengthStringMin до lengthStringMax и для каждой строки подстроку
...
Функции генерации достаточно использовать один раз для создания всех файлов с тестами,
 в дальшейшем они могут не использоваться.


Для тестирования также предусмотрены две функции:

Первая предназначена для тестирования файла со случайными строками и подстроками,
 именно ею тестируется файл **manualTests.txt**.
```c++
...
Вторая предназначена для тестирования набора  строк одинаковой длинны
 для получения среднего времени построения дерева.
```c++
...
## Реализация <p id = "Real"></p>
  - **SuffixTree.h** - header файл с основной описаниями класса Node  и класса SuffixTree.
  - **SuffixTree.cpp** - cpp файл с реализацией функций класса SuffixTree.
  - **Main.cpp** - запускаемый файл с функциями для тестов.
## Использование
Библиотека подключается строкой 
